...
SWEP.Instructions = "Взломщик кейпадов"
SWEP.Category  = "RP"
...
SWEP.KeyCrackSound = Sound("scoreboard/urperror.ogg")
...
if(CLIENT) then
	...
	function SWEP:DrawHUD()
		if self.IsCracking then
			...
			draw.RoundedBox(6, x, y, w, h, sup.color.background_alpha)	
			draw.RoundedBox(6, x + 5, y + 5, frac * (w - 10), h - 10, Color(255 + (frac * -255), frac * 255, 40))
			
			surface.SetFont("defont24")
			local fontw, fonth = surface.GetTextSize("Взлом")
			local fontx, fonty = (x + (w / 2)) - (fontw / 2), (y + (h / 2)) - (fonth / 2)
			
			surface.SetTextPos(fontx + 1, fonty+1)
			surface.SetTextColor(color_black)
			surface.DrawText("Взлом"..dots)
	...
	net.Receive("KeypadCracker_Sparks", function()
		...
		if IsValid(ent) then
			...
			ent:EmitSound("buttons/combine_button7.wav", 50, 100)
		end
	end)
end
